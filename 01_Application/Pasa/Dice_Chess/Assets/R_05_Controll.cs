...
// 直前の位置を記録する。
public class R_05_BU : MonoBehaviour
{
    public static Vector3 vector3 = new Vector3();
}

// このキューブの動作をコントロールする。
public class R_05_Controll : MonoBehaviour
{
        ...
        else if ((Cube_Status.Status_R_05 == true) && (other.gameObject.tag == "Red_Cube"))
        {
            ...
            if (transform.localScale.y < 0.5f)
            {
                ...
                if (other.gameObject.transform.localScale.y < 0.5f)
                {
                    ...
                    Cube_Status.OddEven_Cube[Define.R_05] = false;
                    // doubleステータスをtrueにする
                    Cube_Status.Double_Cube[Define.R_05] = true;
                    // エフェクトを切る
                    GetComponent<ParticleSystem>().Stop();
                }
                else
    ...
    void Update()
    {
        ...
        Cube_Location.Cube_Loc[Define.R_05] = transform.position;
