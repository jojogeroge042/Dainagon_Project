...
// 直前の位置を記録する。
public class R_10_BU : MonoBehaviour
{
    public static Vector3 vector3 = new Vector3();
}

// このキューブの動作をコントロールする。
public class R_10_Controll : MonoBehaviour
{
    GameObject CUBE_R10_KING;

    // Start is called before the first frame update
    void Start()
    {
       this.CUBE_R10_KING = GameObject.Find("R_Cube_King");
    }
    ...
    {
        //　自操作時、敵キューブと衝突したとき
        if ((Cube_Status.Status_R_10 == true) && (other.gameObject.tag == "Blue_Cube"))
        {
          if(Game_Status.Kill == false)
          {
            ...
            if (other.gameObject.transform.localScale.y < 0.5f)
            {
                ...
                // 奇数偶数ステータスをfalseにする
                Cube_Status.OddEven_Cube[Define.R_10] = false;
                // エフェクトを切る
          ...
          {
            // 元の位置に戻す
            transform.position = B_10_BU.vector3;
        ...
        // 味方キューブの場合
        else if ((Cube_Status.Status_R_10 == true) && (other.gameObject.tag == "Red_Cube"))
        {
            // 自キューブが二段積みではない場合
            if (transform.localScale.y < 0.5f)
            {
                ...
                if (other.gameObject.transform.localScale.y < 0.5f)
                {
                    ...
                    Cube_Status.OddEven_Cube[Define.R_10] = false;
                    // doubleステータスをtrueにする
                    Cube_Status.Double_Cube[Define.R_10] = true;
                    // エフェクトを切る
                    GetComponent<ParticleSystem>().Stop();
                }
                else
                {
                    // 元の位置に戻す
                    transform.position = R_10_BU.vector3;
                    if (Dice_Status.Count == 0)
                    {
                      GetComponent<ParticleSystem>().Play();
                    }
    ...
    void Update()
    {
                    ...
                    if ((((Local_x - x) == 0) && ((Local_z - z) == 1))
                      || (((Local_x - x) == 0) && ((Local_z - z) == -1))
                        || (((Local_x - x) == 1) && ((Local_z - z) == 0))
                          || (((Local_x - x) == -1) && ((Local_z - z) == 0)))
                    {
                        R_10_BU.vector3 = transform.position;
        ...
        Cube_Location.Cube_Loc[Define.R_10] = transform.position;
        if (transform.localScale.y < 0.5f)
        {
          this.CUBE_R10_KING.transform.position = new Vector3(transform.position.x, transform.position.y + 0.25f, transform.position.z);
        }
        else
        {
          this.CUBE_R10_KING.transform.position = new Vector3(transform.position.x, transform.position.y + 0.50f, transform.position.z);
        }
        if (Dice_Status.Count == 0)
        {
          GetComponent<ParticleSystem>().Stop();
