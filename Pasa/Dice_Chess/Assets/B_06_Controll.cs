...
// 直前の位置を記録する。
public class B_06_BU : MonoBehaviour
{
    public static Vector3 vector3 = new Vector3();
}

// このキューブの動作をコントロールする。
    public class B_06_Controll : MonoBehaviour
{
    ...
    {
        //　自操作時、敵キューブと衝突したとき
        if ((Cube_Status.Status_B_06 == true) && (other.gameObject.tag == "Red_Cube"))
        {
          ...
          {
            // 元の位置に戻す
            transform.position = B_06_BU.vector3;
        ...
        // 味方キューブの場合
        else if ((Cube_Status.Status_B_06 == true) && (other.gameObject.tag == "Blue_Cube"))
        {
            // 自キューブが二段積みではない場合
    ...
    void Update()
    {
                    ...
                    if ((((Local_x - x) == 0) && ((Local_z - z) == 1))
                      || (((Local_x - x) == 0) && ((Local_z - z) == -1))
                        || (((Local_x - x) == 1) && ((Local_z - z) == 0))
                          || (((Local_x - x) == -1) && ((Local_z - z) == 0)))
                    {
                        B_06_BU.vector3 = transform.position;
        ...
        Cube_Location.Cube_Loc[Define.B_06] = transform.position;
