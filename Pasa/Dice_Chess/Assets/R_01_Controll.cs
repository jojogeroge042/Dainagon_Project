...
// 直前の位置を記録する。
public class R_01_BU : MonoBehaviour
{
    public static Vector3 vector3 = new Vector3();
}

// このキューブの動作をコントロールする。
public class R_01_Controll : MonoBehaviour
{
    ...
    {
        //　自操作時、敵キューブと衝突したとき
        if ((Cube_Status.Status_R_01 == true) && (other.gameObject.tag == "Blue_Cube"))
        {
          if(Game_Status.Kill == false)
          {
            ...
            if (other.gameObject.transform.localScale.y < 0.5f)
            {
                ...
                // 奇数偶数ステータスをfalseにする
                Cube_Status.OddEven_Cube[Define.R_01] = false;
                // エフェクトを切る
          ...
          {
            // 元の位置に戻す
            transform.position = R_01_BU.vector3;
        ...
        // 味方キューブの場合
        else if ((Cube_Status.Status_R_01 == true) && (other.gameObject.tag == "Red_Cube"))
        {
            // 自キューブが二段積みではない場合
            if (transform.localScale.y < 0.5f)
            {
                ...
                if (other.gameObject.transform.localScale.y < 0.5f)
                {
                    ...
                    Cube_Status.OddEven_Cube[Define.R_01] = false;
                    // doubleステータスをtrueにする
                    Cube_Status.Double_Cube[Define.R_01] = true;
                    // エフェクトを切る
                    GetComponent<ParticleSystem>().Stop();
                }
                else
    ...
    void Update()
    {
                    ...
                    if ((((Local_x - x) == 0) && ((Local_z - z) == 1))
                      || (((Local_x - x) == 0) && ((Local_z - z) == -1))
                        || (((Local_x - x) == 1) && ((Local_z - z) == 0))
                          || (((Local_x - x) == -1) && ((Local_z - z) == 0)))
                    {
                        R_01_BU.vector3 = transform.position;
        ...
        Cube_Location.Cube_Loc[Define.R_01] = transform.position;
